...
exports.updateMe = async (req, res) => {
    try {
        ...
        const [rows] = await db.query('SELECT id, name, npm, email, phone, role, is_active FROM users WHERE id = ? LIMIT 1', [userId]);
...
exports.logout = (req, res) => {
    try {
        ...
        if (req.user) {
            addLog({ user_id: req.user.id, role: req.user.role, action: 'LOGOUT', table_name: 'users', record_id: req.user.id, description: `User logged out - ID: ${req.user.id}` });
        }
    } catch (err) {
...
exports.register = async (req, res) => {
    try {
        ...
        const [existing] = await db.query(
            ...
            console.log('Register attempt:', { email, npm, origin: req.headers.origin || req.ip });
        ...
        try {
            ...
            if (err && err.code === 'ER_DUP_ENTRY') {
                return res.status(409).json({ message: 'Email atau NPM sudah terdaftar.' });
            }
            console.error('Database error creating user:', err);
            return res.status(500).json({ message: 'Gagal mendaftarkan akun. Silakan coba lagi.' });
        }

        const newUserId = result.insertId;

            console.log('New user registered:', { id: newUserId, email, npm, origin: req.headers.origin || req.ip });
        await addLog({
            user_id: newUserId,
            role: 'mahasiswa',
            action: 'REGISTER',
    ...
    try {
        ...
        if (!match) {
            return res.status(400).json({ message: 'Email atau password salah.' });
                ...
                console.log('Login attempt:', { email, origin: req.headers.origin || req.ip });
